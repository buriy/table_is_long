...
from .labels import save_fasttext, vallabel, Labels
...
def prepare_df(df, get_text):
    ...
    if 'TextFT' not in df.columns:
        df['TextFT'] = [get_text(frame) for frame in PRINTER.itertuples(df)]
    return df


class FastTextTrain:
    def __init__(self, modelset, get_text):
        self.modelset = modelset
        self.root = self.modelset.root
    ...
    def save_subsets(self, df, cols_all, test_size=50000, overwrite=False):
        self.modelset.save_labels()
        df = prepare_df(df, self.get_text)
        ft = []
        for frame in PRINTER.itertuples(df):
            ft.append([self.modelset.labels.by_value[col, getattr(frame, col)].label for col in cols_all])
        ...
        for sname, sdf in self.get_subsets(df_ft, test_size=test_size):
            PRINTER.print("Preparing subset", sname.format('Col'))
...
class FastTextPredict:
    ...
    def predict_cols(self, texts, cols):
        ...
        for x in PRINTER.iterate(texts):
            for c in cols:
                pred = self.modelset.models[c].predict(x)
                prob = self.modelset.models[c].predict_p(x)
    ...
    def predict_df(self, df, cols):
        df = prepare_df(df, self.get_text)
        self.modelset.preload_parallel(df.TextFT, cols)
